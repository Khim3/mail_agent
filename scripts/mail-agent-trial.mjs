...
async function main() {
  ...
  console.log("Expected server setting: MAIL_SEND_DRY_RUN=true");
  ...
  for (let i = 1; i <= trials; i += 1) {
    ...
    const recipientMailAddress = Array.from(new Set(newLogs));
    seenLogCount = logs.length;

    const trialEntry = { recipientMailAddress };
    report.results.push(trialEntry);
    writeReport();

    console.log(`attempt ${i}: ${JSON.stringify(trialEntry)}`);
  }

  console.log(`Recipient log saved: ${RECIPIENT_LOG_PATH}`);
